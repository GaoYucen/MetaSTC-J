...
for dist_func in dist_functions:
    ...
    np.savetxt('STC-ML/param/meta/feature_used_label_dis_'+str(look_forward)+'_'+str(k)+'.txt', feature_used_label)

    #%%
    # 读取feature_used_label
    feature_used_label = np.loadtxt('STC-ML/param/meta/feature_used_label_dis_'+str(look_forward)+'_'+str(k)+'.txt')
    # 统计各项出现的次数
    ...
    #%% 存储模型参数
    torch.save(maml.model.state_dict(), 'STC-ML/param/meta/maml_model_dis_'+str(look_forward)+'.pth')
    ...
    #%% 读取模型参数
    for i in range(k):
        maml.model.load_state_dict(torch.load('STC-ML/param/meta/maml_model_dis_'+str(look_forward)+'.pth'))

        for data, targets in train_loader_['train_loader_'+str(i)]:
            data, targets = data.to(device), targets.to(device)
            maml.inner_loop(data, targets)

        torch.save(maml.model.state_dict(), 'STC-ML/param/meta/maml_model_dis_'+str(look_forward)+'_'+str(i)+'.pth')

        maml.model.load_state_dict(torch.load('STC-ML/param/meta/maml_model_dis_'+str(look_forward)+'_'+str(i)+'.pth'))
