...
def kmeans(data, k, dist_function):
    """
    ...
    while True:
        ...
        # 使用cos作为距离函数
        if dist_function == 'cos':
            # distances = np.dot(data, centroids.T) / (np.linalg.norm(data) * np.linalg.norm(centroids.T, axis=0))
            # print(distances.shape)
            cos_sim = cosine_similarity(data, centroids).T
            distances = 1 - cos_sim
            ...
            for centroid in centroids:
                ...
                for datum in data:
                    ...
                    # 计算DTW距离
                    distance, _ = fastdtw(datum, centroid, dist=euclidean)
...
dist_functions = ['euclidean', 'cos', 'dtw']
for dist_func in dist_functions:
    feature_used_label = kmeans(feature_used_new, k, dist_function=dist_func)
    np.savetxt('STC-ML/param/meta/feature_used_label_' + str(look_forward) + '_' + str(dist_func) + '_' + str(k) + '.txt', feature_used_label)
    # feature_used_label = np.loadtxt('STC-ML/param/meta/feature_used_label_' + str(look_forward) + '_' + str(dist_func) + '_' + str(k) + '.txt')
    ...
    test_dataset_ = {}
    for i in range(k):
        train_dataset_['train_dataset_' + str(i)] = TensorDataset(train_x_['train_x_' + str(i)],
                                                                  train_y_['train_y_' + str(i)])
        test_dataset_['test_dataset_' + str(i)] = TensorDataset(test_x_['test_x_' + str(i)],
                                                                test_y_['test_y_' + str(i)])
    ...
    test_loader_ = {}
    for i in range(k):
        train_loader_['train_loader_' + str(i)] = DataLoader(train_dataset_['train_dataset_' + str(i)],
                                                             batch_size=batch_size, shuffle=True)
        test_loader_['test_loader_' + str(i)] = DataLoader(test_dataset_['test_dataset_' + str(i)],
    ...
    # %% 存储模型参数
    torch.save(maml.model.state_dict(), 'STC-ML/param/meta/maml_model_' + str(look_forward) + '_' + str(dist_func) + '.pth')
    ...
    # %% 读取模型参数
    for i in range(k):
        maml.model.load_state_dict(torch.load('STC-ML/param/meta/maml_model_' + str(look_forward) + '_' + str(dist_func) + '.pth'))

        for data, targets in train_loader_['train_loader_' + str(i)]:
            data, targets = data.to(device), targets.to(device)
            maml.inner_loop(data, targets)

        torch.save(maml.model.state_dict(), 'STC-ML/param/meta/maml_model_' + str(look_forward) + '_' + str(dist_func) + '_' + str(i) + '.pth')

        maml.model.load_state_dict(
            torch.load('STC-ML/param/meta/maml_model_' + str(look_forward) + '_' + str(dist_func) + '_' + str(i) + '.pth'))
    ...
    f = open('STC-ML/log/meta-LSTM' + '_' + str(dist_func)+'.txt', 'a')
