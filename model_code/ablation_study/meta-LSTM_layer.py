...
class LSTMModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.linear = nn.Linear(look_back * hidden_dim, look_back * hidden_dim)
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(look_back * hidden_dim, look_back * hidden_dim)
        self.relu2 = nn.ReLU()
        self.linear3 = nn.Linear(look_back * hidden_dim, look_forward * output_dim)
...
np.savetxt('STC-ML/param/meta/feature_used_label_layer_'+str(look_forward)+'_'+str(k)+'.txt', feature_used_label)

#%%
# 读取feature_used_label
feature_used_label = np.loadtxt('STC-ML/param/meta/feature_used_label_layer_'+str(look_forward)+'_'+str(k)+'.txt')
# 统计各项出现的次数
...
#%% 存储模型参数
torch.save(maml.model.state_dict(), 'STC-ML/param/meta/maml_model_layer_'+str(look_forward)+'.pth')
...
#%% 读取模型参数
for i in range(k):
    maml.model.load_state_dict(torch.load('STC-ML/param/meta/maml_model_layer_'+str(look_forward)+'.pth'))

    for data, targets in train_loader_['train_loader_'+str(i)]:
        data, targets = data.to(device), targets.to(device)
        maml.inner_loop(data, targets)

    torch.save(maml.model.state_dict(), 'STC-ML/param/meta/maml_model_layer_'+str(look_forward)+'_'+str(i)+'.pth')

    maml.model.load_state_dict(torch.load('STC-ML/param/meta/maml_model_layer_'+str(look_forward)+'_'+str(i)+'.pth'))
