...
def hippo_recurrence(inputs_p: torch.Tensor, A: torch.Tensor, B: torch.Tensor, c_init: torch.Tensor):
    """
    Optimized HiPPO recurrence using TorchScript to eliminate Python loop overhead.
    """
    L = inputs_p.size(0)
    B_size = inputs_p.size(1)
    C = inputs_p.size(2)
    N = A.size(0)
    c = c_init
    cs = torch.empty(L, B_size, C, N, device=inputs_p.device)
    B_expanded = B.unsqueeze(0)
    AT = A.t()
    for t in range(L):
        f = inputs_p[t].unsqueeze(-1)
        new = f @ B_expanded
        c = c @ AT + new
        cs[t] = c
    return cs
...
class HiPPO_LegT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        L = inputs.size(-1)
        B, C = inputs.shape[:2]
        c_init = torch.zeros(B, C, self.N, device=inputs.device)
        inputs_p = inputs.permute(2, 0, 1) # (L, B, C)
        return hippo_recurrence(inputs_p, self.A, self.B, c_init)
...
class SpectralConv1d(nn.Module):
    ...
    def forward(self, x):
        ...
        # MPS backend has limited support for ComplexFloat and FFT operations.
        # We perform these operations on CPU to ensure compatibility and avoid RuntimeErrors.
        ...
        w_real, w_imag = self.weights_real, self.weights_imag
        if orig_device.type == 'mps':
            w_real, w_imag = w_real.cpu(), w_imag.cpu()

        out_ft[:, :, :, :self.modes] = self.compl_mul1d("bjix,iox->bjox", a, w_real, w_imag)
    ...
    def inner_loop(self, data, targets):
        with torch.enable_grad():
            for step in range(self.n_inner_loop):
                ...
                # Use FOMAML: create_graph=False for massive speedup
...
#%% Add argument parser
parser = argparse.ArgumentParser(description='Meta-FiLM model for traffic flow prediction')
parser.add_argument('--use_cpu', action='store_true', help='Force use of CPU')
parser.add_argument('--epochs', type=int, default=50, help='Number of training epochs')
parser.add_argument('--full_data', action='store_true', help='Train on full dataset without batch sampling')
...
os.makedirs('param/meta-film', exist_ok=True)
...
print(f'Total parameters in FiLM Model: {total_params}')
...
max_batches = 20 if not args.full_data else float('inf')

print(f"Starting full performance test for {num_epochs} epochs...")
for epoch in range(num_epochs):
    ...
    for i in range(k):
        for j, (data, targets) in enumerate(train_loader_['train_loader_'+str(i)]):
            ...
            count += 1
            if j >= max_batches - 1: break
    print(f'Epoch [{epoch+1}/{num_epochs}] completed in {time.time() - epoch_start:.2f}s | Batches: {count}')
...
#%% 读取模型参数
print("Starting fine-tuning and evaluation...")
for i in range(k):
    ...
    for j, (data, targets) in enumerate(train_loader_['train_loader_'+str(i)]):
        ...
        if not args.full_data and j >= 10: break # 微调阶段也支持快速采样
...
# 确定展示的时间步长度
num_test_steps = time_len - train_size - look_back - look_forward
display_steps = min(num_test_steps, 288) 

# 抽取第一个路段的预测结果
indices = np.arange(0, display_steps * look_forward, look_forward)

if len(preds_list) >= indices[-1]:
    plt.plot(targets_list[indices], label='Actual Speed', color='#4C72B0', linewidth=2, alpha=0.8)
    plt.plot(preds_list[indices], label='Predicted Speed (FiLM)', color='#C44E52', linewidth=1.5, linestyle='--')
    
    plt.title(f'Traffic Speed Prediction Comparison (Meta-FiLM)', fontsize=14)
...
plt.savefig('graph/MetaSTC_FiLM_Prediction.pdf')
print("预测曲线图已保存至 graph/MetaSTC_FiLM_Prediction.pdf")
plt.show()
