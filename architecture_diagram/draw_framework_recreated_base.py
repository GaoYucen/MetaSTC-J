...
def create_framework_diagram():
    ...
    # -- Spatial Branch --
    ax.text(8.0, 9.4, "Spatial Features", ha='center', fontsize=12, color=COLORS['text'])
    
    # Feature boxes
    feat_w, feat_h = 1.4, 0.6
    f1 = draw_shadow_box(ax, 6.2, 8.6, feat_w, feat_h, "Width", facecolor='white', edgecolor=COLORS['border_middle'], boxstyle='round,pad=0.02')
    f2 = draw_shadow_box(ax, 7.8, 8.6, feat_w, feat_h, "Length", facecolor='white', edgecolor=COLORS['border_middle'], boxstyle='round,pad=0.02')
    f3 = draw_shadow_box(ax, 9.4, 8.6, feat_w, feat_h, "#(Lanes)", facecolor='white', edgecolor=COLORS['border_middle'], boxstyle='round,pad=0.02')
    ...
    # -- Temporal Branch --
    t2v = draw_shadow_box(ax, 7.0, 1.5, 3.0, 0.7, "Traffic2Vec", facecolor='#E1BEE7')
    t_enc = draw_shadow_box(ax, 7.0, 2.7, 3.0, 0.7, "Temporal Encoding", facecolor='#E1BEE7', fontweight='bold')
    draw_fancy_arrow(ax, t2v['x'], t2v['top'], t_enc['x'], t_enc['bottom'], color=COLORS['border_middle'])

    # -- Gating --
    gating = draw_shadow_box(ax, 7.5, 4.5, 2.0, 0.9, "Gating", facecolor='#CE93D8', fontweight='bold', fontsize=12)
    ...
    ax_scatter.axis('off')
    ax.text(13.5, 6.5, "Clusters", fontsize=13, fontweight='bold', color=COLORS['text'])
    ...
    ax.text(13.5, 2.8, "Clustering Representation", ha='center', fontsize=12, color=COLORS['text'])
    ...
    # ==================== 3. Conditional Meta Learner (Right) ====================
    # Background Area
    ...
    ax.text(20.15, 10.2, "Conditional Meta Learner", ha='center', fontsize=16, fontweight='bold', color=COLORS['text'])

    # Memory
    mem = draw_shadow_box(ax, 17.5, 8.0, 1.8, 1.0, "Memory", facecolor=COLORS['box_memory'], fontsize=11, fontweight='bold')
    
    # Params
    p_cluster = draw_shadow_box(ax, 19.8, 8.2, 1.0, 0.6, r"$\theta^{cluster}$", facecolor='white')
    p_theta = draw_shadow_box(ax, 21.0, 8.2, 0.8, 0.6, r"$\theta$", facecolor='#FFF9C4')
    p_phi = draw_shadow_box(ax, 22.0, 8.2, 0.8, 0.6, r"$\phi$", facecolor='#FFF9C4')

    draw_fancy_arrow(ax, mem['right'], mem['y'], p_cluster['left'], p_cluster['y'], color=COLORS['border_right'])
    draw_fancy_arrow(ax, p_phi['left'], p_phi['y'], p_theta['right'], p_theta['y'], color=COLORS['border_right'])

    # Meta Learner Blocks (Repeated)
    y_start = 6.5
    y_step = 2.2
    ...
    for i in range(3):
        ...
        # Meta Learner Box
        ml = draw_shadow_box(ax, 17.5, y_pos, 1.8, 1.0, f"Meta\nLearner {i+1 if i<2 else 'K'}", 
                            facecolor=COLORS['box_meta'], fontweight='bold')
        ...
        # Vertical connections (simplified)
        if i == 0:
            # Init to first layer
            draw_fancy_arrow(ax, p_cluster['x'], p_cluster['bottom'], 19.8 + 0.5, y_pos+1.0, color=COLORS['border_right']) # To CG
            draw_fancy_arrow(ax, p_theta['x'], p_theta['bottom'], 20.5, y_pos+1.0, color=COLORS['border_right'])
            draw_fancy_arrow(ax, p_phi['x'], p_phi['bottom'], 22.6, y_pos+1.0, color=COLORS['border_right']) # To Update
        ...
        if i < 2:
            ...
            # Dots
            if i == 1:
                ax.text(18.4, y_pos - 1.1, ":", fontsize=16, fontweight='bold', color=COLORS['text'])
                ax.text(20.7, y_pos - 1.1, ":", fontsize=16, fontweight='bold', color=COLORS['text'])
    ...
    # Middle -> Right
    # Clustering Rep -> Meta Learner
    # Use a long arrow with connection style
    draw_fancy_arrow(ax, 15.5, 2.2, 17.5, 2.2, color='#757575', lw=2, ls='--') # To Meta Learner K
    draw_fancy_arrow(ax, 15.5, 2.2, 17.5, 6.6, color='#757575', lw=2, ls='--', connectionstyle="arc3,rad=-0.2") # To Meta Learner 1
    
    # Spatio-Temporal h -> Memory
    draw_fancy_arrow(ax, 15.5, 8.4, 17.5, 8.4, color='#757575', lw=2, ls='--')
